...
private:
  ...
  void pickPhysicalDevice() {
    ...
    vkEnumeratePhysicalDevices(instance->getVkInstance(), &deviceCount,
                               nullptr);
  ...
  SwapChainSupportDetails querySwapChainSupport(VkPhysicalDevice device) {
    ...
    vkGetPhysicalDeviceSurfaceFormatsKHR(device, surface->getSurface(),
                                         &formatCount, nullptr);

    if (formatCount != 0) {
      details.formats.resize(formatCount);
      vkGetPhysicalDeviceSurfaceFormatsKHR(
          device, surface->getSurface(), &formatCount, details.formats.data());
    }
  ...
  QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device) {
    ...
    for (const auto &queueFamily : queueFamilies) {
      ...
      vkGetPhysicalDeviceSurfaceSupportKHR(device, i, surface->getSurface(),
                                           &presentSupport);
