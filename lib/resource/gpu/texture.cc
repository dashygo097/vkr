...
void Texture::createEmpty() {
  ...
  if (desc_.layout != VK_IMAGE_LAYOUT_UNDEFINED &&
    ...
    ImageCommands::transitionLayout(device_, command_pool_, *image_,
                                    desc_.image.layout, desc_.layout);
  }
}
...
void Texture::createFromFile() {
  ...
  ImageCommands::copyBufferToImage(device_, command_pool_, staging, *image_, 1,
                                   channels);
...
void Texture::createCubemapFromFiles() {
  ...
  ImageCommands::transitionLayout(device_, command_pool_, *image_,
                                  VK_IMAGE_LAYOUT_UNDEFINED,
                                  VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
  ImageCommands::copyBufferToImage(device_, command_pool_, staging, *image_,
                                   FaceCount, channels);
